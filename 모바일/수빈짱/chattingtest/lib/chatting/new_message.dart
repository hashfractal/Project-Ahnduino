...
class _NewMessageState extends State<NewMessage> {
        ...
        return AlertDialog(
          ...
          content: IntrinsicHeight(
            child: Column(
              ...
              children: <Widget>[
                const Text('사진 업로드',textAlign: TextAlign.center,
                ),
                const Padding(
  ...
  Future<void> _sendMessage() async {
    FocusScope.of(context).unfocus();
    String uniqueID =  DateFormat('yyyyMMddHHmmss').format(DateTime.now().toUtc().add(const Duration(hours: 9)));
    ...
    if(assetList != null)
      {
        for (int i = 0 ; i< assetList!.length ; i ++ ) {
          ...
          await FirebaseStorage.instance
              .ref()
              .child(chat.email.toString()).child('chat')
              .child(uniqueID).child("image$i"+'.png').putFile(F!);
          String url = await FirebaseStorage.instance
              .ref()
              .child(chat.email.toString()).child('chat')
              .child(uniqueID).child("image$i"+'.png').getDownloadURL();
          sendMap['image$i'] = url;
        }
      }


    FirebaseFirestore.instance.collection('chat').doc('chat').collection(chat.email.toString()).doc(uniqueID).set(sendMap);
    FirebaseFirestore.instance.collection('chat').doc('chat').collection('needanswer').doc(chat.email.toString()).set(
        {'time':DateTime.now()});
    _controller.clear();
    _userEnterMessage='';
  }
